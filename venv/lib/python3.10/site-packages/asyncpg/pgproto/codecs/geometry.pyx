...
cdef inline _encode_point(WriteBuffer wbuf, object point):
    cdef object point
...
cdef inline _decode_point(FRBuffer *buf):
    cdef:
...
cdef box_encode(CodecContext settings, WriteBuffer wbuf, obj):
    wbuf.write_int32(32)
    _encode_point(wbuf, (obj[0], obj[1]))
...
cdef path_decode(CodecContext settings, FRBuffer *buf):
    ...
    return pgproto_types.Path(*_decode_point(buf), is_closed=is_closed == 1)
...
cdef poly_decode(CodecContext settings, FRBuffer *buf):
    return pgproto_types.Polygon(*_decode_point(buf))
