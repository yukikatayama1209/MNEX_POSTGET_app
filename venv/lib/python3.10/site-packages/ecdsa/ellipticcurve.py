...
class CurveFp(object):
    ...
    if GMPY:  # pragma: no branch
        ...
        def __init__(self, p, a, b, h=None):
            """
            The curve of point satisfying y^2 = x^3 + a*x + b (mod p).

            h is an integer that is the cofactor of the elliptic curve domain
            parameters; it is the number of point satisfying the elliptic
...
class CurveEdTw(object):
    ...
    if GMPY:  # pragma: no branch
        ...
        def __init__(self, p, a, d, h=None, hash_func=None):
            """
            The curve of point satisfying a*x^2 + y^2 = 1 + d*x^2*y^2 (mod p).
...
class AbstractPoint(object):
    """Class for common methods of elliptic curve point."""
    ...
    def to_bytes(self, encoding="raw"):
        ...
        by `encoding="raw"`. It can also output point in :term:`uncompressed`,
        :term:`compressed`, and :term:`hybrid` formats.

        For point on Edwards curves `encoding` is ignored and only the
...
class PointJacobi(AbstractPoint):
    ...
    def __eq__(self, other):
        """Compare for equality two point with each-other.

        Note: only point that lay on the same curve can be equal.
        """
    ...
    def __ne__(self, other):
        """Compare for inequality two point with each-other."""
        return not self == other
    ...
    def from_affine(point, generator=False):
        ...
        :param bool generator: set to True to make the point to precalculate
          ...
          signatures (around 100 or so) or for generator point of a curve.
        """
        return PointJacobi(
    ...
    def _add_with_z_1(self, X1, Y1, X2, Y2, p):
        """add point when both Z1 and Z2 equal 1"""
        # after:
    ...
    def _add(self, X1, Y1, Z1, X2, Y2, Z2, p):
        """add two point, select fastest method."""
        if not Y1 or not Z1:
            return X2, Y2, Z2
        if not Y2 or not Z2:
            return X1, Y1, Z1
    ...
    def __mul__(self, other):
        ...
        _add = self._add
        # since adding point when at least one of them is scaled
    ...
    def mul_add(self, self_mul, other, other_mul):
        ...
        if not isinstance(other, PointJacobi):
            other = PointJacobi.from_affine(other)
        # when the point have precomputed answers, then multiplying them alone
        ...
        # as we have 6 unique point to work with, we can't scale all of them,
        ...
        # so with 2 point, we have 9 combinations:
        # 0, -A, +A, -B, -A-B, +A-B, +B, -A+B, +A+B
        # so we need 4 combined point:
...
class Point(AbstractPoint):
    ...
    def __init__(self, curve, x, y, order=None):
        ...
        # for curves with cofactor 1, all point that are on the curve are
    ...
    def __eq__(self, other):
        """Return True if the point are identical, False otherwise.
    ...
    def __ne__(self, other):
        """Returns False if point are identical, True otherwise."""
        return not self == other
...
class PointEdwards(AbstractPoint):
    ...
    def __eq__(self, other):
        ...
        Note: only point on the same curve can be equal.
        """
    ...
    def _add(self, X1, Y1, Z1, T1, X2, Y2, Z2, T2, p, a):
        """add two point, assume sane parameters."""
