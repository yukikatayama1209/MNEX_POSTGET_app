...
class WorkingSet:
    ...
    def iter_entry_point(self, group, name=None):
        """Yield entry point objects from `group` matching `name`

        If `name` is None, yields all entry point in `group` from all
    ...
    def parse_map(cls, data, dist=None):
        ...
        for group, lines in data:
            if group is None:
                if not lines:
                    continue
                raise ValueError("Entry point must be listed in groups")
            group = group.strip()
            if group in maps:
    ...
    def get_entry_map(self, group=None):
        ...
        try:
            ...
            ep_map = self._ep_map = EntryPoint.parse_map(
                self._get_metadata('entry_point.txt'), self
            )
...
def _initialize_master_working_set():
    ...
    iter_entry_point = working_set.iter_entry_point
